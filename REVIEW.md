...
Исправленные файлы лежат рядом с оригиналами: `code-review/TimesheetReportHandler.fixed.cs` и `code-review/TimeEntriesPage.fixed.tsx`. Это полные версии, а не наброски: агрегация вынесена в MongoDB, деньги переведены на `decimal`, ставка резолвится по дате, процент отсутствует при нулевом бюджете, на клиенте типизированный контракт, серверные фильтры и итоги, пагинация, удаление с версией и валидация формы. В комментариях проставлены номера строк этой таблицы, чтобы каждое исправление сопоставлялось с дефектом. Production-реализация находится в `backend/` и `frontend/`.
